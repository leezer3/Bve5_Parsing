...
namespace Bve5_Parsing.MapGrammar
{
    ...
    internal class MapGrammarErrorStrategy : DefaultErrorStrategy
    {
        ...
        {
            var msg = ErrorMessage.GetMessage(ParseMessageType.FailedPredicate, FilePath, GetTokenErrorDisplay(e.OffendingToken));
            NotifyErrorListeners(recognizer, msg, e);
        }

        protected override void ReportInputMismatch([NotNull] Parser recognizer, [NotNull] InputMismatchException e)
        {
            var msg = ErrorMessage.GetMessage(ParseMessageType.InputMismatch, FilePath, GetTokenErrorDisplay(e.OffendingToken), e.GetExpectedTokens().ToString(recognizer.Vocabulary));
            NotifyErrorListeners(recognizer, msg, e);
        }
        ...
        protected override void ReportMissingToken([NotNull] Parser recognizer)
        {
            ...
            var msg = ErrorMessage.GetMessage(ParseMessageType.MissingToken, FilePath, GetTokenErrorDisplay(t), expecting.ToString(recognizer.Vocabulary));
        ...
        {
            var msg = ErrorMessage.GetMessage(ParseMessageType.NoViable, FilePath, GetTokenErrorDisplay(e.OffendingToken), e.GetExpectedTokens().ToString(recognizer.Vocabulary));
            NotifyErrorListeners(recognizer, msg, e);
        }
